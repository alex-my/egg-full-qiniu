...
class FullQiniu {
  ...
  async _OP(style, key) {
    return new Promise((resolved, reject) => {
      ...
      bucketManager[style](this.bucket, key, function (err, respBody, respInfo) {
        if (err) {
          reject(err);
        } else {
          if (respInfo.statusCode !== 200 && this.isLog) {
  ...
  async changeType(key, newType) {
    return new Promise((resolved, reject) => {
      const bucketManager = this._getBucketManager();
      bucketManager.changeType(this.bucket, key, newType, function (err, respBody, respInfo) {
        if (err) {
          reject(err);
        } else {
          if (respInfo.statusCode !== 200 && this.isLog) {
  ...
  async _MC(style, srcKey, destBucket, destKey, isForce) {
    return new Promise((resolved, reject) => {
      ...
      bucketManager[style](this.bucket, srcKey, destBucket, destKey, options, function (err, respBody, respInfo) {
        if (err) {
          reject(err);
        } else {
          if (respInfo.statusCode !== 200 && this.isLog) {
  ...
  async deleteAfterDays(key, days) {
    return new Promise((resolved, reject) => {
      const bucketManager = this._getBucketManager();
      bucketManager.deleteAfterDays(this.bucket, key, days, function (err, respBody, respInfo) {
        if (err) {
          reject(err);
        } else {
          if (respInfo.statusCode !== 200 && this.isLog) {
