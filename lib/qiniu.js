...
class FullQiniu {
  ...
  _getPutPolicy() {
    const needCreate = (this.putPolicy === null || this.putPolicy.expires > )
  ...
  _getMac() {
    if (this.mac === null) {
      this.mac = new qiniu.auth.digest.Mac(qiniu.conf.ACCESS_KEY, qiniu.conf.SECRET_KEY);
    }
    return this.mac;
  }
  ...
  _upload(key, file, style) {
    ...
    return new Promise((resolved, reject) => {
      formUploader[style](token, key, file, putExtra, (e, respBody, respInfo) => {
        if (this.isLog) {
        ...
        if (e) {
          this.app.coreLogger.error(`[egg-full-qiniu] _upload, e: ${JSON.stringify(e)}`);
          reject(e);
        } else {
